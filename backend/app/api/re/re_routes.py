...
@router.post("/re/insert-json", status_code=status.HTTP_201_CREATED)
async def upload_json_mongo(
...
async def delete_by_ids_endpoint_mongo(file: UploadFile = File(...), db=Depends(get_db)):
    if file.content_type not in ("application/json", "text/json"):
        raise HTTPException(status_code=400, detail="File must be JSON format")
    raw = await file.read()
...
@router.get("/re/get-ids", status_code=status.HTTP_200_OK)
async def get_all_ids_mongo(db=Depends(get_db)):
    try:
        collection = db[COLLECTION_NAME]
        ids = []
